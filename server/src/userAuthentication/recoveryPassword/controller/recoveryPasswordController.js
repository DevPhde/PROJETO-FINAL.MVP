...
export class recoveryPasswordController extends recoveryPasswordUseCase {
    ...
    static recoveryPassword = async (req, res) => {
        ...
        const sendNewPassword =  newPassword.status ? await this.mailer.sendMail("recoveryPassword", data, `sua nova senha é: ${newPassword.message}`) : res.status(409).send(newPassword);
